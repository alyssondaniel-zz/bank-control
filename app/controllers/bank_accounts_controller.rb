class BankAccountsController < ApplicationController
  ...
  def change_current_account
    respond_to do |format|
      if current_user.update(current_bank_account_id: params["bank_account_id"])
        format.html { redirect_to transactions_path, notice: 'Bank account was successfully updated.' }
      else
  ...
  private
    ...
    def bank_account_params
      params.require(:bank_account).permit(:bank_agency_id, :number, :limit, :user_id)
    end

    def set_user_id
      params["bank_account"]["user_id"] = current_user.id
    end
end
